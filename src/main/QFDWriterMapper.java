...
public class QFDWriterMapper extends Mapper<RequestReplyMatch,
	...
	public void map(RequestReplyMatch record, NullWritable ignore, Context ctxt)
		...
		md.update(record.getDestIp().getBytes(StandardCharsets.UTF_8));
		hash = md.digest();	
		...
		md = HashUtils.cloneMessageDigest(messageDigest);
		md.update(record.getCookie().getBytes(StandardCharsets.UTF_8));
		hash = md.digest();
		hashBytes = Arrays.copyOf(hash, HashUtils.NUM_HASH_BYTES);
		hashString = DatatypeConverter.printHexBinary(hashBytes);
		key = new WTRKey("cookie", hashString);
		ctxt.write(key, record);
//`		System.out.println("above is for QFDWriterMapper");
    }
}
